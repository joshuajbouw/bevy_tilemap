...
impl AsRef<(i32, i32)> for Point2 {
    fn as_ref(&self) -> &(i32, i32) {
        &self.0
    }
}

impl AsMut<(i32, i32)> for Point2 {
    fn as_mut(&mut self) -> &mut (i32, i32) {
        &mut self.0
    }
}
    ...
    ($vec: ty) => {
        impl From<$vec> for Point2 {
            fn from(vec: $vec) -> Point2 {
                Point2((vec.x() as i32, vec.y() as i32))
            }
        }
    ...
    ($arr: ty) => {
        impl From<$arr> for Point2 {
            fn from(arr: $arr) -> Point2 {
                Point2((arr[0] as i32, arr[1] as i32))
            }
        }
...
impl AddAssign for Point2 {
    fn add_assign(&mut self, rhs: Self) {
        *self = Point2((self.x() + rhs.x(), self.y() + rhs.y()))
    }
}
    ...
    ($vec: ty) => {
        impl From<$vec> for Point3 {
            fn from(vec: $vec) -> Point3 {
                Point3((vec.x() as i32, vec.y() as i32, vec.z() as i32))
            }
        }
...
macro_rules! point3_arr_impl {
    ($vec: ty) => {
        impl From<$vec> for Point3 {
            fn from(vec: $vec) -> Point3 {
                Point3((vec[0] as i32, vec[1] as i32, vec[2] as i32))
            }
        }
...
impl AddAssign for Point3 {
    fn add_assign(&mut self, rhs: Self) {
        *self = Point3((self.x() + rhs.x(), self.y() + rhs.y(), self.z() + rhs.z()))
    }
}
